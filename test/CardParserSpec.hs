    ...
    describe "parseCard" $ do
        describe "normal cards" $ do
            it "parses basic card" $ do
                stringToCard "2h" `shouldBe` Just (NormalCard Two Heart 1 None Base Nothing)

            it "parses card with quantity" $ do
                stringToCard "2h3" `shouldBe` Just (NormalCard Two Heart 3 None Base Nothing)

            it "parses card with enhancement" $ do
                stringToCard "2hw" `shouldBe` Just (NormalCard Two Heart 1 Wild Base Nothing)

            it "parses card with edition" $ do
                stringToCard "2hf" `shouldBe` Just (NormalCard Two Heart 1 None Foil Nothing)

            it "parses card with seal" $ do
                stringToCard "2hr" `shouldBe` Just (NormalCard Two Heart 1 None Base (Just Red))

            it "parses full card" $ do
                stringToCard "2h3wfy" `shouldBe` Just (NormalCard Two Heart 3 Wild Foil (Just GoldS))
        ...
        describe "stone cards" $ do
            ...
            it "parses stone card with quantity" $ do
                stringToCard "s3" `shouldBe` Just (StoneCard 3 Base Nothing)

            it "parses stone card with edition" $ do
                stringToCard "sf" `shouldBe` Just (StoneCard 1 Foil Nothing)

            it "parses stone card with seal" $ do
                stringToCard "sr" `shouldBe` Just (StoneCard 1 Base (Just Red))

            it "parses full stone card" $ do
                stringToCard "s3fy" `shouldBe` Just (StoneCard 3 Foil (Just GoldS))
