...
instance Aeson.ToJSON EvaluateRequest where
    toJSON (EvaluateRequest cards size distance) = Aeson.object
        [ "cards" Aeson..= cards
        , "size" Aeson..= size
      ...
      where
        parseCard = withObject "Card" $ \v -> do
            ...
            if map toUpper rankStr == "S" && map toUpper suitStr == "S"
              then return $ Card Ace Spade Stone (parseEdition editionStr) (parseSeal sealStr)
              else case (parseRank rankStr, parseSuit suitStr) of
                    (Just r, Just s) -> return $ Card r s (parseEnhancement enhancementStr) (parseEdition editionStr) (parseSeal sealStr)
        ...
        post "/api/simulate" $ do
            ...
            case Aeson.decode requestBody of
                Just SimulateRequest{..} -> do
                    ...
                    if V.null deckCards then do
                        status status400
                        json $ object ["error" .= ("Invalid deck format" :: String)]
                    else if V.null handCards then do
                        status status400
